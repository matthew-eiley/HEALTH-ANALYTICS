...
def do_sleep_section(stats):
    ...
    with col1:
        ...
        if avg_sufficiency >= 85:
            ...
            st.warning(f"⚠️ Poor. My average sleep sufficiency this week was {avg_sufficiency:.2f}%")
...
def main_app():
    ...
    tab1, tab2 = st.tabs(["🛏️ **Sleep Performance**", "🥇 **Recovery & Strain**"])
    
    with tab1:
        st.markdown("""
        <div class="section-header">
            <h2 class="section-title">🛏️ Sleep Performance Analysis</h2>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab2:
        st.markdown("""
        <div class="section-header">
            <h2 class="section-title">🥇 Recovery and Strain Analysis</h2>
        </div>
        """, unsafe_allow_html=True)
